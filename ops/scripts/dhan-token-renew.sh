...
#
# SCHEDULE
# Daily, INCLUDING weekends (ops/cron/option-decode-dhan-token-renew). A
# token left un-renewed over a Saturday is expired by Monday, and an expired
# token cannot be renewed at all - only replaced by hand.
#
# Usage:
#   dhan-token-renew.sh --dry-run   # check and report, no destructive call
#   dhan-token-renew.sh             # renew, persist, restart
...
if left_h <= 0:
    print("ERR token already expired %.1fh ago - must be regenerated by hand at web.dhan.co" % -left_h); raise SystemExit
print("OK %.2f" % left_h)
PY
)
case "$PREFLIGHT" in
