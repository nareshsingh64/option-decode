...
/**
 * Calendar-style expiry picker used everywhere the dashboard lets you pick
 * an option expiry (Market Controls, Replay Lab). Unlike a plain <select>,
 * this only lets you land on dates that actually have stored snapshots for
 * the current underlying - every other day on the calendar is rendered but
 * disabled, so it's visually obvious which expiries exist without having
 * to scroll a flat dropdown list.
 *
...
function formatDisplayDate(value: string): string {
  ...
  if (!date) return value || "Select expiry";
  return date.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
}

export function ExpiryDatePicker({ expiries, value, onChange, name, disabled }: ExpiryDatePickerProps) {
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  const availableSet = useMemo(() => new Set(expiries), [expiries]);
  const sortedExpiries = useMemo(() => [...expiries].sort(), [expiries]);

  const fallbackAnchor = parseIsoDate(value) ?? parseIsoDate(sortedExpiries[0] ?? "") ?? new Date();
  ...
  // picker opens, so switching underlyings while it's closed doesn't leave
  // it showing a stale month next time it's opened.
  useEffect(() => {
    if (!isOpen) return;
    const anchor = parseIsoDate(value) ?? parseIsoDate(sortedExpiries[0] ?? "") ?? new Date();
    setViewYear(anchor.getFullYear());
    setViewMonth(anchor.getMonth());
    // Only re-anchor on open, not on every value/expiries change while open -
    // otherwise picking a date in an adjacent month would immediately snap
    // the calendar away right as it closes.
  ...
  // stored expiries, so there's no way to page into an empty month with
  // nothing selectable.
  const monthKeys = sortedExpiries.map((expiry) => {
    ...
    <div className="relative" ref={containerRef}>
      ...
      {isOpen ? (
        <div className="absolute left-0 top-full z-20 mt-1 w-64 rounded border border-terminal-line bg-terminal-panel p-3 shadow-lg">
          ...
          {!sortedExpiries.length ? <p className="mt-2 text-xs text-terminal-muted">No stored expiries available yet.</p> : null}
        </div>
      ) : null}
    </div>
  );
