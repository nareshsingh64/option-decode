...
async function startWorker() {
  ...
  const worker = new BullWorker<MarketSnapshotJobData>(
    ...
    async (job: Job<MarketSnapshotJobData>) => {
      ...
      // Serialised against the wave screener's own workers - see
      // heavy-job-lock.ts for the measurement that motivated it. Four BullMQ
      // workers share this process and each being concurrency:1 says nothing
      // about them running against each other, so two ~1GB native
      // allocations were routinely live at once.
      return runExclusive(label, async () => {
