...
## Checking retention is healthy

**Read the `ticks` figure, not a snapshot count.** `pruneMarketDataBefore` has
two loops on two different cutoffs: ticks and pressure scores go at
`SNAPSHOT_RETENTION_DAYS` (30), but the `OptionChainSnapshot` rows themselves
go at `SPOT_PRICE_RETENTION_DAYS` (**180**), so a snapshot's metadata
deliberately outlives its detail. `snapshots: 0` in the completion log is
correct, not a fault.

```bash
sudo zgrep -A8 'Snapshot retention cleanup completed' \
  /opt/option-decode-native/logs/worker/worker.log* | grep -E 'ticks:|detailCutoff'
```

Healthy is ~4M ticks per weekday run. Counting
`OptionChainSnapshot WHERE tradingDate < now - 30 days` measures a cutoff that
does not apply to those rows — that number only grows, and on 2026-08-29 it was
briefly mistaken for the prune having stopped when it was completing daily.
